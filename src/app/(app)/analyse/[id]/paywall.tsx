...
import { unlockDdForTest } from "../actions";
...
export function DdPaywall({
  ...
  projectId: string;
  docs: DocState[];
  /** Wenn true (Admin-Whitelist per ENV): zusätzlicher Test-Bypass-Button. */
  isAdmin?: boolean;
}) {
  ...
  function onTestBypass() {
    ...
    setError(null);
    startBypass(async () => {
      const result = await unlockDdForTest(projectId);
      if (result.error) {
