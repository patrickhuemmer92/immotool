...
/**
 * Aktiv-Status-Widget für die DD-Detail-Seite.
 *
 * Zeigt nur an, wenn tatsächlich Arbeit im Hintergrund läuft:
 *   - Ein Dokument mit ocr_status = "pending"
 *   - Eine gerade laufende Analyse (getriggert per Prop `analyzeRunning`)
 *
 * Pollt alle 2s, solange etwas pending ist. Bei fertigem Job:
 * automatisch `router.refresh()` → die Server-Component lädt die
 * frischen Daten und dieses Widget verschwindet.
 */
    ...
    async function tick() {
      ...
      try {
        const res = await fetch(
          `/api/dd/status?dd_project_id=${encodeURIComponent(projectId)}`,
          { cache: "no-store" }
        );
        if (!res.ok) return;
        ...
        // Wenn ein Dokument gerade fertig geworden ist → Server-Refresh,
        // aber nur einmal pro Dokument (via Timestamp-Fingerprint).
