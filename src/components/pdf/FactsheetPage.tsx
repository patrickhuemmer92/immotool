...
/**
 * Single-page property Factsheet — extracted from FactsheetDocument so the
 * Portfolio-Factbook can append the same Factsheet for every property
 * (instead of the old 7-page-per-property scheme).
 *
 * Layout intentionally matches the standalone Factsheet PDF: header with
 * brand line, optional cover image, KPI strip, two-column section with
 * owners/tenant/valuation/loans/latest-pnl.
 */
...
const styles = StyleSheet.create({
  ...
  label: { color: pdfColors.muted, flexShrink: 1 },
  value: { fontFamily: "Inter-Bold", textAlign: "right", flexShrink: 0 },
  footer: {
    position: "absolute",
    bottom: 24,
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.twoCol}>
          ...
          <View style={styles.section}>
            ...
            {data.tenant ? (
              <>
                ...
                {data.tenant.contract_start && (
                  <View style={styles.row}>
                    <Text style={styles.label}>
                      {t("tenants.contract_start")}
                    </Text>
                    <Text style={styles.value}>{data.tenant.contract_start}</Text>
                  </View>
                )}
                {data.tenant.is_fixed_term && data.tenant.contract_end && (
                  <View style={styles.row}>
                    <Text style={styles.label}>
                      {t("tenants.contract_end")}
                    </Text>
                    <Text style={styles.value}>{data.tenant.contract_end}</Text>
                  </View>
                )}
                ...
                {data.tenant.ancillary_costs_per_month != null && (
                  <View style={styles.row}>
                    <Text style={styles.label}>
                      {t("tenants.ancillary_costs_per_month")}
                    </Text>
                    <Text style={styles.value}>
                      {eur(data.tenant.ancillary_costs_per_month)}
                    </Text>
                  </View>
                )}
              </>
        ...
        <View style={styles.col}>
            ...
            {data.loans.length === 0 ? (
              ...
              data.loans.map((l) => (
                <View key={l.id} style={styles.row}>
                  ...
                  <Text style={styles.value}>{eur(l.remaining_balance)}</Text>
                </View>
              ))
            )}
            {data.loans.length > 0 && (
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>{t("factsheet.latest_pnl")}</Text>
            {data.latestPnL ? (
              <>
                <View style={styles.row}>
                  <Text style={styles.label}>{t("factsheet.period")}</Text>
